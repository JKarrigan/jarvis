...
function CollectionRow({
  ...
  return (
    <div className="flex flex-col gap-3 md:flex-row md:gap-6">
      <div className="shrink-0 md:w-[208px] md:pl-[var(--rail)] md:pt-2">
        <p className="text-[11px] font-bold uppercase tracking-[0.16em]" style={{ color: collColor(hue) }}>Collection</p>
        <h2 className="mt-1 text-[clamp(24px,4vw,32px)] font-[800] leading-[1.05] tracking-[-0.02em] text-ink">{name}</h2>
        <Link href={`/media/collections/${id}`} className="mt-1 inline-flex items-center gap-1 text-sm text-white/55 transition hover:text-white">
          {count} film{count === 1 ? '' : 's'} <ChevronRightIcon className="h-3.5 w-3.5" />
        </Link>
      </div>
...
export function CollectionsList({ catalog, franchises }: { catalog: ReelTitle[]; franchises: CollectionSummary[] }) {
  const { customCollections, createCollection } = useMedia()
  ...
  const liveFranchises = franchises.map(f => ({ ...f, titles: resolve(f.itemIds) })).filter(f => f.titles.length > 0)

  return (
    <div className="py-10">
      <div className="mb-8 pl-[var(--rail)] pr-[var(--gx)]">
        <h1 className="text-[clamp(34px,8vw,58px)] font-[800] tracking-[-0.025em] text-ink">Collections</h1>
        <p className="mt-2 text-white/55">Franchises and sagas, grouped for a marathon.</p>
      </div>

      {liveFranchises.length > 0 && (
        <section className="space-y-12">
          {liveFranchises.map(f => (
            <CollectionRow key={f.id} id={f.id} name={f.name} hue={f.hue} count={f.titles.length} titles={f.titles} />
          ))}
        </section>
      )}
      ...
      <section className="mt-16">
        <div className="mb-4 pl-[var(--rail)] pr-[var(--gx)]">
          <h2 className="text-[22px] font-bold text-ink">Your collections</h2>
          <form
            onSubmit={(e) => { e.preventDefault(); if (newName.trim()) { createCollection(newName); setNewName('') } }}
        ...
        {customCollections.length === 0 ? (
          <p className="pl-[var(--rail)] pr-[var(--gx)] text-sm text-white/45">
            No custom collections yet. Create one above, then add titles from any detail page with “Add to collection”.
          </p>
        ) : (
          <div className="space-y-12">
            {customCollections.map(c => (
              <CollectionRow key={c.id} id={c.id} name={c.name} hue={c.hue} count={c.items.length} titles={resolve(c.items)} />
            ))}
          </div>
        )}
      </section>
