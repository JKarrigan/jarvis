...
function EpisodeRow({ ep, index }: { ep: JellyfinEpisode; index: number }) {
  return (
    <motion.div
      ...
      className="flex gap-4 py-3 border-b border-zinc-800 last:border-0"
    >
      <span className="text-zinc-600 text-sm font-mono w-6 shrink-0 pt-0.5 text-right">
        {ep.IndexNumber}
      </span>
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          ...
          {ep.RunTimeTicks && (
            <span className="text-xs text-zinc-500 shrink-0">{formatRuntime(ep.RunTimeTicks)}</span>
          )}
        </div>
        {ep.Overview && (
          <p className="text-xs text-zinc-500 mt-1 line-clamp-2">{ep.Overview}</p>
        )}
      </div>
    </motion.div>
  )
    ...
    <div className="min-h-screen pb-16">
      ...
      <div className="relative w-full" style={{ height: '60vh', minHeight: 380 }}>
        ...
        <motion.div
          ...
          className="absolute top-4 left-4 md:left-6 z-10"
        >
          <Link
            href={backHref}
        ...
        <div className="absolute bottom-0 left-0 right-0 px-4 md:px-6 pb-8">
          ...
          >
            {item.Taglines?.[0] && (
              <p className="text-sm text-zinc-400 italic mb-2">{item.Taglines[0]}</p>
            )}
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-zinc-100 leading-tight max-w-3xl">
              {item.Name}
            </h1>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-3 text-sm text-zinc-400">
              ...
              {item.Status === 'Ended' && (
                <span className="text-xs text-zinc-600 uppercase tracking-wide">Ended</span>
              )}
            </div>
            ...
            >
              <button className="inline-flex items-center gap-2 bg-zinc-100 hover:bg-white text-zinc-900 font-semibold text-sm px-5 py-2.5 rounded-lg transition-colors">
                <svg viewBox="0 0 16 16" className="w-4 h-4 fill-current">
                  <path d="M3 2.5a.5.5 0 01.768-.422l10 5.5a.5.5 0 010 .844l-10 5.5A.5.5 0 013 13.5v-11z" />
                </svg>
                Play
              </button>
              <button className="inline-flex items-center gap-2 border border-zinc-600 hover:border-zinc-400 text-zinc-300 hover:text-zinc-100 text-sm px-4 py-2.5 rounded-lg transition-colors">
                <svg viewBox="0 0 16 16" className="w-4 h-4 fill-current">
      ...
      {/* Content */}
      <div className="px-4 md:px-6 mt-8 space-y-10">
        {/* Genres */}
          ...
          <div className="max-w-2xl">
            <AnimatePresence initial={false}>
              <motion.p
                ...
                className={`text-sm text-zinc-300 leading-relaxed ${!overviewExpanded ? 'line-clamp-3' : ''}`}
              >
                {item.Overview}
              </motion.p>
            </AnimatePresence>
        ...
        {actors.length > 0 && (
          <section>
            <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-4">Cast</h2>
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide -mx-4 md:-mx-6 px-4 md:px-6">
              {actors.map((person, i) => (
                ...
                >
                  <div
                    className="w-16 h-16 rounded-full flex items-center justify-center text-lg font-bold text-white/70 ring-2 ring-white/10"
                  ...
                  <div className="text-center">
                    ...
                    {person.Role && (
                      <p className="text-[11px] text-zinc-600 mt-0.5 line-clamp-1">{person.Role}</p>
                    )}
                  </div>
                </motion.div>
              ))}
        ...
        {item.Type === 'Series' && item.Seasons && item.Seasons.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider">Episodes</h2>
        ...
        {moreItems.length > 0 && (
          <section>
            <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-4 px-0">
              More Like This
            </h2>
