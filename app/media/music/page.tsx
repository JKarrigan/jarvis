...
import { MOCK_MUSIC } from '@/app/_components/media/mockData'

export default function MusicPage() {
  const [search, setSearch] = useState('')
  const filtered = MOCK_MUSIC.filter(m =>
    ...
    <main className="py-6 space-y-6">
      <div className="flex items-center justify-between gap-4 px-4 md:px-6">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">{MOCK_MUSIC.length} albums</p>
        </div>
        <input
          type="search"
